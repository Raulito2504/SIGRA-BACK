...
static async getAll({ page = 1, limit = 10, filters = {} }) {
    ...
    static async getWithPolizas() {
        try {
            const [vehiculos] = await pool.execute(`
                ...
                FROM vehiculos v
                LEFT JOIN polizas_seguro ps ON v.id = ps.id_vehiculo
    ...
    static async addDocument(vehiculoId, tipoDocumento, archivo, usuarioId) {
        try {
            const [result] = await pool.execute(
                'INSERT INTO documentos_vehiculo (id_vehiculo, tipo_documento, nombre_archivo, ruta_archivo, subido_por) VALUES (?, ?, ?, ?, ?)',
                [vehiculoId, tipoDocumento, archivo, `/uploads/${archivo}`, usuarioId]
            );
            return result.insertId;
        } catch (error) {
    ...
    static async getDocuments(vehiculoId) {
        try {
            const [documentos] = await pool.execute('SELECT * FROM documentos_vehiculo WHERE id_vehiculo = ?', [vehiculoId]);
            return documentos;
        } catch (error) {
